    ...
    app.get('/vote/:name', function (req, res) {
        ...
        try {
            sql.all('select * from users cross join temps where users.tempId=temps.id and users.id=' + uid + ';', function (err, rows) {
                ...
                res.render(isMobile(req.headers['user-agent']) ? 'pages/wap_mail2.html' : 'pages/wap_mail2.html', { layout: null, sha: ranStr.sha, tz: 0, uid: uid, mid: mid, bgUrl: bgUrl, ranStr: ranStr.sha, ranImg: ranStr.ranImg, ranRp: ranStr.ranRp });
                ranStr.count++;
            });
        } catch (e) {
